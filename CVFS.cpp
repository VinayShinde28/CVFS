...
#define MAXINODE 50     // MACRO

#define READ 1      // 1 means permission to read
#define WRITE 2     // 3 means permission to read & write
...
typedef struct inode        //inode does't have file name // every file system have directry which contains file name and inode number
{ 
    char FileName[50];
    int InodeNumber;
    int FileSize;
...
{
    int readoffset;     // when we read this will increase
    int writeoffset;
    int count;          // 1    it gets increase when we create child process
    int mode;   //1 2 3     // when we open
    PINODE ptrinode; 
}FILETABLE,*PFILETABLE;

typedef struct ufdt
{
    PFILETABLE ptrfiletable;        // Actual OS has ufdt srtucture
}UFDT;

UFDT UFDTArr[MAXINODE];     //index of this is fd
...
void man(char *name)
{
    ...
    {
        printf("Description : Used to display infrmation of file\n");
    ...
    {
        printf("Description : Used to rempve data from file\n");
...
int CreateFile(char *name,int permission)
{
    ...
    UFDTArr[i].ptrfiletable->ptrinode->ReferenceeCount = 1;
...
int OpenFile(char *name , int mode)
{
    ...
    UFDTArr[i].ptrfiletable->ptrinode =temp;
    (UFDTArr[i].ptrfiletable->ptrinode->ReferenceeCount)++;
...
void closeFileByName(int fd)
{
    ...
    (UFDTArr[fd].ptrfiletable->ptrinode->ReferenceeCount)--;
}
...
int fstat_file(int fd)  // information about the file
{
    PINODE temp = head;
    int i =0;
...
int main()
{
    ...
    InitialiseSuperBlock();         // oxillary data
    CreateDILB();           // oxillary data
    ...
    while(1)    //Shell
    {
        ...
        else if(count == 3)
        {
            if(strcmp(command[0],"create") == 0)
            {
               ...
               // strcpy(UFDTArr[i].ptrfiletable->ptrinode->FileName,command[1]);
                ret = CreateFile(command[1],atoi(command[2]));
                if(ret >= 0)
                    printf("File is successfully created with file descriptoe : %d\n",ret);
                if(ret == -1)
                    printf("ERROR : Incorrect parameters\n");
                if(ret == -2)
            ...
            else if(strcmp(command[0],"open") == 0)
            {
                ...
                if(ret >= 0)
                    printf("File is successfully opened with file descriptoe : %d\n",ret);
                if(ret == -1)
                    printf("ERROR : Incorrect parameters\n");
                if(ret == -2)
